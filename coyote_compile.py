...
def main():
    ...
    action_group.add_argument(
        "-l", "--list", action="store_true", help="List available benchmarks and exit"
    )
    action_group.add_argument(
        "-c", "--circuits", type=str, nargs="+", help="Compile the specified circuits"
    )

    parser.add_argument(
        "-o", "--output-dir", type=str, help="Where to place Coyote output", default="."
    )
    parser.add_argument(
        "--backend-dir",
        type=str,
        help="Location of Coyote backend",
        default="./bfv_backend",
    )
    parser.add_argument(
        "--combine",
        action="store_true",
        help="Combine everything into a single circuit",
    )

    cpp_group = parser.add_mutually_exclusive_group()
    cpp_group.add_argument(
        "--no-cpp", action="store_true", help="Stop before generating C++"
    )
    cpp_group.add_argument(
        "--just-cpp",
        action="store_true",
        help="Use precompiled Coyote code to directly generate C++",
    )
    ...
    parser.add_argument(
        ...
        "--update-cmake",
        action="store_true",
        help="Update CMakeLists.txt with circuits",
    )
    ...
    for circuit in circuits:
        ...
        if args.just_cpp:
            if not (
                os.path.exists(os.path.join(output_dir, circuit))
                and os.path.exists(os.path.join(output_dir, circuit, "scal"))
                and os.path.exists(os.path.join(output_dir, circuit, "vec"))
            ):
                parser.error(f"No precompiled circuit found for `{circuit}`")
            print(f"Using precompiled circuit for `{circuit}`")
            scalars[circuit] = open(
                os.path.join(output_dir, circuit, "scal")
            ).readlines()
            vectors[circuit] = open(
                os.path.join(output_dir, circuit, "vec")
            ).readlines()
        else:
    ...
    if not args.no_cpp:
        for circuit in circuits:
            ...
            open(
                os.path.join(args.backend_dir, "coyote_out", circuit, "scalar.cpp"), "w"
            ).write(scalar_cpp)
            open(
                os.path.join(args.backend_dir, "coyote_out", circuit, "vector.cpp"), "w"
            ).write(vector_cpp)
