...
{
    return {"decision_tree_packed_partially(cs):1", "1", "decision_tree_packed_partially(cs):0", "decision_tree_packed_partially(os):5", "decision_tree_packed_partially(cs):2", "decision_tree_packed_partially(os):4", "decision_tree_packed_partially(os):3", "decision_tree_packed_partially(os):0", "decision_tree_packed_partially(os):1", "decision_tree_packed_partially(os):2"};
}
...
std::vector<ctxt> ScalarProgram::computation(std::map<std::string, ctxt> locs, RuntimeContext &info)
{
    ...
    regs[0] = locs["decision_tree_packed_partially(cs):0"];
    regs[1] = locs["decision_tree_packed_partially(cs):1"];
    regs[2] = locs["decision_tree_packed_partially(os):0"];
    info.eval->multiply(regs[1], regs[2], regs[3]);
    info.eval->relinearize_inplace(regs[3], rk);
    info.eval->add(locs["1"], regs[1], regs[4]);
    regs[5] = locs["decision_tree_packed_partially(cs):2"];
    regs[6] = locs["decision_tree_packed_partially(os):1"];
    info.eval->multiply(regs[5], regs[6], regs[7]);
    info.eval->relinearize_inplace(regs[7], rk);
    info.eval->add(locs["1"], regs[5], regs[8]);
    regs[9] = locs["decision_tree_packed_partially(os):4"];
    ...
    regs[16] = locs["decision_tree_packed_partially(cs):2"];
    regs[17] = locs["decision_tree_packed_partially(os):2"];
    info.eval->multiply(regs[16], regs[17], regs[18]);
    info.eval->relinearize_inplace(regs[18], rk);
    info.eval->add(locs["1"], regs[16], regs[19]);
    regs[20] = locs["decision_tree_packed_partially(cs):1"];
    regs[21] = locs["decision_tree_packed_partially(os):3"];
    info.eval->multiply(regs[20], regs[21], regs[22]);
    info.eval->relinearize_inplace(regs[22], rk);
    info.eval->add(locs["1"], regs[20], regs[23]);
    regs[24] = locs["decision_tree_packed_partially(os):5"];
