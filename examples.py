...
def matmul(a, b):
    return [
        recursive_sum([a[i][k] * b[k][j] for k in range(len(a))])
...
def conv(sig, ker):
    ...
    for offset in range(len(sig) - len(ker) + 1):
        output.append(
            recursive_sum([sig[offset + i] * ker[i] for i in range(len(ker))])
        )
    return output
...
def sort_3(c12, c23, c13, o123, o132, o213, o231, o312, o321):
    return cond(
        c12,
        (cond(c23, o123, cond(c13, o132, o312))),
        (cond(c13, o213, cond(c23, o231, o321))),
    )
...
def sort_3_packed(cs, os):
    return cond(
        cs[0],
        (cond(cs[1], os[0], cond(cs[2], os[1], os[4]))),
        (cond(cs[2], os[2], cond(cs[1], os[3], os[5]))),
    )
...
print("\n".join(scalar_code))
print("=" * 20)
print("\n".join(map(str, vector_code.instructions)))
