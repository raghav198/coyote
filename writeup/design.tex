...
The bound is initially very loose but we iteratively tighten it until the solver returns UNSAT, meaning no smaller schedule could be found, so the most recent one was the smallest. \raghav{better?}\milind{yup.}
...
\milind{You say ``these dependencies'' here, but there's no mention of dependences earlier in this section :-) Also, you definitely need to explain wha the lane placement problem {\em is} before you explain how you solve it... Just make sure that's part of your TODO above :-) Also, I definitely prefer ``dependence'' and ``dependences'' as the words to use here...}
