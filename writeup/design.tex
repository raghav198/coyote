...
For a program that is split into $k$ epochs, we can represent these dependencies \milind{You say ``these dependencies'' here, but there's no mention of dependences earlier in this section :-) Also, you definitely need to explain wha the lane placement problem {\em is} before you explain how you solve it... Just make sure that's part of your TODO above :-) Also, I definitely prefer ``dependence'' and ``dependences'' as the words to use here...} in a $k$-partite graph, where each partition corresponds to a epoch and each vertex in a partition corresponds to a subexpression computed in that epoch.
