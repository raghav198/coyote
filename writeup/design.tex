...
This effectively greedily selects a set of expressions to vectorize, which amounts to scheduling the first {\em phase} of the program, s we want to repeat the process until the entire program has been scheduled.
Repeating the process requires some extra work: whenever a set of subexpressions is selected, each one needs to be ``quotiented out'' of the original program (i.e. replaced by a single leaf node in the expression tree).
Doing so has two effects on the vectorizability graph:
\begin{enumerate}
    \item All the nodes that were cut out of the expression tree (the {\em sub}-subexpressions) need to be eliminated, since they have already been scheduled.
...
Given a set of subexpressions to compute in a single phase of the program, we can align the instructions between subexpressions to actually produce a vector schedule.\milind{why ``phase'' here instead of ``stage''?}\raghav{I was getting in the habit of calling it a phase everywhere, since ``stage'' is kind of overloaded (staging compiler). If stage makes more sense, though, I can go back to it. }
It may seem like the solution to this is just sequence alignment, but aligning trees is actually more complicated than that.
...
This causes the dynamic programming approach of sequence alignment to quickly blow up.
