...
\subsection{Overview}
The vector schedule \raghav{To be pedantic, this is kind of a ``pre-schedule'', where we haven't done any vertical alignment yet. Mostly because that's both too easy to worry about and too hard to worry about. But what do I say here?} is represented as a labeled quotient of the original circuit graph, where each vertex represents a connected subgraph, and is labeled with an integer representing the lane the subgraph gets placed on, satisfying the condition that no two vertices at the same height are labeled by the same lane.
...
If there are no available arcs to contract (for example, if all available arcs have already been chosen and rejected), the algorithm restarts at the best graph seen so far.
...
Since each step of this algorithm involves an expensive call to the lane placement procedure, and because the space of graph quotients is highly discrete \raghav{I have a vague intuition about the space I'm trying to convey here but I don't know how to say it properly}, it runs for a much smaller number of rounds, usually between 150 and 200.
