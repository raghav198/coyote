...
\begin{enumerate}
    ...
    \item In general, the only way to move data between vector slots is by rotating the entire vector. This makes it much more important to assign vector lanes to packed instructions optimally, since realizing arbitrary permutations by composing several rotations quickly gets computationally expensive. \milind{Maybe say something explicit here about how composing rotations quickly blows up the multiplicative depth of the circuit?}
\end{enumerate}
...
We discuss the specific drawbacks of applying the SLP approach to FHE in Section~\ref{sec:related-work}. \raghav{forward reference good?}\milind{yup.}
