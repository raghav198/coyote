...
\begin{enumerate}
    \item The vectors are much larger than traditional hardware vector registers (e.g. several thousand slots wide, compared to the usual 4 or 8 slots). Utilizing this much space poses unique challenges. \raghav{Should I expand on this?}
...
\end{enumerate}
The challenges posed by points (2) and (3) in particular preclude us from simply using SLP-style vectorization, since it fails to take the high cost of data movement between lanes into account when deciding what instructions to pack together. \raghav{I don't know what to say about VeGen here, since its not like any of the things about FHE really ``stop'' us from using it, just that it doesn't account for as much as we'd like it to?}
