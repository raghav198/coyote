...
\milind{need an intro paragraph here -- you should never dive right into subsections.
Some suggestions:
1. What is this section going to talk about?
2. Explain that we implement \system using an edsl in python to express computations, stage it down to circuits, evaluate the circuits, create C, etc. It's basically a recap of the first paragraph of overview... In fact, you could change the first paragraph of Overview to instead basically be ``Coyote is an eDSL and compiler for converting arithmetic expressions written in a high level language into arithmetic circuits, and then compiling those arithmetic circuits into vectorized FHE code.'' and put the more detailed paragraph here.}
