...
Both of these input controls allow programmers to trade off between predictable input layouts and flexible input layouts. Predictable layouts are good for composition---different kernels can be plugged together more easily, without incurring rotations to line the kernels up---but may be bad for performance---\system has less flexibility to manage how the inputs flow through the kernel.
