...
For smaller sizes, we see \system's vectorization is capable of matching or even outperforming the expert-written baselines, although on larger sizes the search space is often too big to automatically find the expert schedules.
Manually inspecting the generated code shows that this is usually because the schedule \system generates uses one or two more rotates than the baseline.
In the case of the dot product, the schedules \system finds all use the same number of rotates as the expert schedule, but occasionally incur more blends.
...
\begin{description}
    ...
    \item[Rows/Cols:] The rows of $A$ are packed into three separate vectors, as are the columns of $B$.
    \item[Cols/Rows:] The columns of $A$ are packed into three separate vectors, as are the rows of $B$.
