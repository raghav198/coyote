...
\begin{enumerate}
    ...
    \item {\bf How well do \system's schedules compare to hand-optimized code?} We compare the run times of various hand-optimized benchmarks to the run time of the vector code that \system generates for them. This is in Section~\ref{sec:expert-comparison}
\end{enumerate}
...
Each of these kernels has a well-known expert-optimized baseline implementation, which we also implemented in \system's vector IR, before compiling both implementations to C++ and measuring the run times.
The results are shown in Table~\ref{tab:expert-comparison-numbers}.

\begin{table}
    \small
    \caption{Table comparing \system vectorization to expert-written code}
    \begin{tabular}{lcc}
        \toprule
...
\begin{table}
    \small
    \caption{Table showing compilation time and vector operation counts for the data layout case study}\label{tab:bigass-data-layout}
    % \vspace{-1em}
    \begin{tabular}{lcccccccc}
    \toprule
    Benchmark & Time (s) & VAdd+VSub & VMul & Rot & Blend\\\midrule
    mm.3.case1 & 584     & 5         & 2    & 15  & 10\\
    mm.3.case2 & 594     & 5         & 2    & 20  & 10\\
    mm.3.case3 & 565     & 11        & 6    & 24  & 22\\
    mm.3.case4 & 601     & 5         & 3    & 24  & 10\\
    mm.3.case5 & 522     & 16        & 13   & 33  & 25\\\bottomrule
    \end{tabular}
\end{table}
...
When multiple entries are on one vector, \system can arrange the elements such that rotating one automatically gives useful rotations of the others (see Section~\ref{sec:duplicating-inputs}).
...
This effect can be seen in Table~\ref{tab:bigass-data-layout}: the layout in case 5 incurs over twice as many rotates as the layout in case 1!
