...
class copy_matrix:
    ...
    def __getitem__(self, i):
        if i >= self.rows:
            raise IndexError("list index out of range")
        return copy_vector(
            self.cols, f"{self.name}:{i}", var_conv=f"{self.name}:{i};{{i}}"
        )
...
class coyote_compiler:
    ...
    def instantiate(self, *funcs, _input_groups=None, _force_lanes=None):
        ...
        return [" ".join(f"%{reg}" for reg in self.outputs)] + list(
            map(str, self.compiler.code)
        )
    ...
    def get_outputs(self, funcs):
        ...
        if len(funcs) == 0:
            ...
            funcs = list(
                filter(lambda func: func.__name__ in funcs, self.func_signatures.keys())
            )
            ...
            for _p, t in signature.items():
                ...
                if isinstance(t, matrix):
                    if t.replicate:
                        ...
                        params[_p] = [
                            [Var(f"{p}:{i};{j}") for i in range(t.rows)]
                            for j in range(t.cols)
                        ]
                    input_groups.append(
                        {f"{p}:{i};{j}" for i in range(t.rows) for j in range(t.cols)}
                    )
                elif isinstance(t, vector):

                    if t.replicate and t.pack:
                        raise TypeError(
                            "`replicate` and `pack` options not allowed together!"
                        )
        ...
        def wrapper(func):
            ...
            for p in types:
                ...
                if not any(isinstance(types[p], t) for t in allowed_types):
                    raise TypeError(
                        f"In function '{name}': '{types[p]}' is not an allowed type"
                    )
            ...
            if any(param.kind == param.VAR_POSITIONAL for param in params.values()):
                raise TypeError(
                    f"In function '{name}': positional arguments not allowed"
                )
            ...
            for p in params:
                if p not in types:
                    types[p] = scalar()
                    warn(
                        f"In function '{name}': No type provided for {p}, assuming scalar"
                    )
