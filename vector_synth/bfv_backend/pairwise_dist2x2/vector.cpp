...
ctxt VectorProgram::computation(std::vector<ctxt> ts, std::map<std::string, ptxt> bits, RuntimeContext &info)
{
    ...
    info.eval->add(vs[6], ss[10], vs[7]); // __v7 = __v6 + __s10
    info.eval->add(vs[6], ss[9], vs[8]); // __v8 = __v6 + __s9
    info.eval->add(vs[6], ss[12], vs[9]); // __v9 = __v6 + __s12
    info.eval->add(vs[6], ss[11], vs[10]); // __v10 = __v6 + __s11
    return vs[10];
}
