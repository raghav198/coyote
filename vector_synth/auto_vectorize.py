...
def dependency_graph(program: List[TAC]) -> List[List[int]]:

    def lookup_line(reg):
        return next(i for i in range(len(program)) if program[i].dest.n == reg.n)
    ...
    for line in program:
        ...
        if line.left.register:
            try:
                line_deps.append(lookup_line(line.left))
            except StopIteration:
                pass
        if line.right.register:
            try:
                line_deps.append(lookup_line(line.right))
            except StopIteration:
                pass
...
def build_vectorized_code(program: List[TAC], schedule: List[int], lanes: List[int], types: z3.ArrayRef, WARP_SIZE) -> List[VecTAC]:
    vectorized_code = []
    ...
    for instrs in inv_schedule:
        ...
        vectorized_code.append(
            VecTAC(dest, left, right, types[i]))

    return vectorized_code


def get_vector_schedule(program: List[TAC], WARP_SIZE, MAX_LENGTH):
    ...
    _steps = z3.Int('steps')
    # how many instructions require a rotate
    _lane_mismatches = z3.Int('lane_mismatches')
    ...
    for i in range(num_instr):
        ...
        opt.add(_schedule[i] < MAX_LENGTH)
    ...
    opt.add(_lane_mismatches == z3.Sum([z3.If(
        _lanes[i] == _lanes[dep], 0, 1) for i in range(num_instr) for dep in dep_graph[i]]))

    # opt.minimize(_steps + 3 * _lane_mismatches)
    opt.set('timeout', 10 * 60 * 1000)  # 10 minutes
    start = time()
    res = opt.check()
    end = time()
    if res == z3.sat:
        print(
            f'Solved! Took {int(1000 * (end - start))} ms')
    ...
    types = [('+' if model[_types][t] ==
              itype.plus else '*') for t in range(num_instr)]
    # steps = model[_steps]
...
if __name__ == '__main__':
    ...
    print('\n'.join(map(str, build_vectorized_code(
        program, *get_vector_schedule(program)))))
    ...
    # opt.add(_lane_mismatches == z3.Sum([z3.If(
    #     _lanes[i] == _lanes[dep.n], 0, 1) for i in range(num_instr) for dep in dep_graph[i]]))
    ...
    # print(
    #     f'Total of {model[_lane_mismatches]} rotations required')
